...
export function CustomerFilters({ params, onChange }: CustomerFiltersProps) {
  ...
  function handleCustomerTypeChange(value: string | null) {
    const resolved = value === 'company' || value === 'individual' ? value : undefined;
    onChange({ ...paramsRef.current, customer_type: resolved, page: 1 });
  }
  ...
  const activeCount = [
    !!params.search, !!params.customer_type, !!params.city, !!params.country, params.is_active !== undefined,
  ].filter(Boolean).length;

  const customerTypeValue = params.customer_type ?? 'all';
  ...
  const typeSelect = (cls: string) => (
    <Select value={customerTypeValue} onValueChange={handleCustomerTypeChange}>
      <SelectTrigger className={cls}><SelectValue placeholder="Tipo" /></SelectTrigger>
      <SelectContent>
